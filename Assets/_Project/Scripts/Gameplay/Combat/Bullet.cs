...
namespace _Project.Scripts.Gameplay.Combat
{
    ...
    public sealed class Bullet : MonoBehaviour, IPoolable
    {
        ...
        private void HandleHit(Collider2D target)
        {
            ...
            _preserveAfterHit = false;
            EnemyController damagedEnemy = FindEnemyTarget(target, damageable);
        ...
        private static EnemyController FindEnemyTarget(Collider2D target, IDamageable damageable)
        {
            if (damageable is EnemyController enemyController)
            {
